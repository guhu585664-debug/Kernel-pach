#!/bin/bash
# android-kernel-choice.sh
# Kullanım: ./android-kernel-choice.sh [PATCH_NAME]
...
# Kullanıcı seçimi
echo -e "${CYN}Seçenekler:${RST}"
echo -e "1) Bootloader Unlock + Kernel Patch"
echo -e "2) Bootloader Relock + Kernel Patch"
...
if [ "$CHOICE" == "1" ]; then
  ...
  echo -e "${CYN}* Bootloader relock başlatılıyor...${RST}"
  echo "$(ts) fastboot flashing lock"
  bar 16 0.04
  echo "(bootloader) Device locked: true"
  echo -e "${GRN}[RESULT] Bootloader: RELOCKED${RST}"
else
  echo -e "${RED}Geçersiz seçim.${RST}"
  exit 1
fi
...
# AVB / dm-verity
echo -e "${CYN}* AVB / dm-verity doğrulama...${RST}"
...
echo "$(ts) fastboot getvar is-unlocked"
if [ "$CHOICE" == "1" ]; then
  echo "(bootloader) is-unlocked: yes"
else
  echo "(bootloader) is-unlocked: no"
fi

echo -e "${GRN}[RESULT] Kernel patch tamamlandı.${RST}"
sleep 0.4
